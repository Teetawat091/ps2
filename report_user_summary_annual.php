...
<script>
	function RelaodPage(){
		window.location.href = "report_user_summary_annual.php?branch_id="+document.getElementById("branch_id").value+"&owner_user_id="+document.getElementById("owner_user_id").value;
	}
...
<form name="form1" action="print_barcode.php" method="post" onsubmit="return doSubmit(this);" target="_blank">
  ...
  <table border='0' style='width:100%; margin-top:15px; font-size:40px;' cellspacing='0' cellpadding='0' align='center' class='bordered'>
    <tr>
      <td colspan="2" align='left' valign='bottom' style='padding-left:30px; font-size:40px;'> สรุปวันลาพักร้อน 	<select name="branch_id" id="branch_id" style="width:150px; font-size:30px;" onchange="RelaodPage()" >
...
$sql = "select * FROM  `dayleave_detail` where `leave_type` like  'annual'  AND user_id in ($_REQUEST[owner_user_id])  Group By `dayleave_id`";
$res = mysql_query($sql);
while($row = mysql_fetch_array($res)){
...
<?
$date1 = $all_date_start[$_REQUEST[owner_user_id]];
$date2 = date('Y-m-d');
...
?>
  <table id='hor-minimalist-b' align="center" class='bordered' >
    ...
    <tbody>
	<? 
	$s_date = $all_date_start[$_REQUEST[owner_user_id]];
