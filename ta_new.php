...
while($row = mysql_fetch_array($res)){
	$un[intval ( $row[user_code])] = $row['title']."".$row['name']." ".$row['sname'];
}
//var_dump($un);
?>
<html>
...
$marr[] = "ธันวาคม";	
?>
<form name="form1" id="form1" action="ta_new.php" method="post" enctype="multipart/form-data" >
...
while (($objArr = fgetcsv($objCSV, 1000, ",")) !== FALSE) {
	if($i > 0){
		...
		/*
		//echo "<br>";
		if(!in_array( $mk_date ,$arr_date[$id])){
			$arr_date[$id][] = $mk_date;
		}
		if(!in_array( $mk_time ,$arr_time[$id][$mk_date])){
			$arr_time[$id][$mk_date][] = $mk_time;
		}
		/**/
...
for($n=0;$n<count($all_id);$n++){
	...
	<tr>
		...
		<td ><? //echo iconv("ISO-8859-11", "UTF-8",$arr[$all_id[$n]]); 
			echo $un[iconv("ISO-8859-11", "UTF-8",$arr_no[$all_id[$n]])];
		?></td>
